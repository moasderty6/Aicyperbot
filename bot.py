...
async def cmd_start(msg: types.Message):
    if not await is_subscribed(msg.from_user.id):
        ...
        return
    await msg.answer("👋 أهلًا بك! هذا البوت مخصص فقط للأسئلة المتعلقة بالأمن السيبراني والاختراق الأخلاقي.\n\nاكتب سؤالك وسأجيبك باستخدام الذكاء الاصطناعي.\n\nاستخدم /sources لمطالعة المصادر.")
...
async def handle_question(msg: types.Message):
    ...
    wait_message = await msg.answer("⏳ جاري إعداد إجابة متخصصة في الأمن السيبراني...")
    ...
    try:
        ...
        prompt = f"""
أنت خبير في الأمن السيبراني. أجب فقط عن الأسئلة المتعلقة بالاختراق الأخلاقي، تحليل البرمجيات الخبيثة، التشفير، الهندسة الاجتماعية، واختبار الاختراق.

❗ إذا كان السؤال خارج هذا النطاق، أجب فقط بالجملة التالية:
⚠️ هذا البوت مختص فقط في مواضيع الأمن السيبراني.
